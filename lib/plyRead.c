...
int readPLY(char filename[], int *nPolygons, Polygon **plist, Color **clist, int estNormals) {
	...
	if(fp) {
		...
		for(i=0;i<numVertex;i++) {
			fscanf(fp, "%f", &(vertex[i].x));
			fscanf(fp, "%f", &(vertex[i].y));
			fscanf(fp, "%f", &(vertex[i].z));
			vertex[i].h = 1.0;

			fscanf(fp, "%f", &(normal[i].x));
			fscanf(fp, "%f", &(normal[i].y));
			fscanf(fp, "%f", &(normal[i].z));
			normal[i].h = 0.0;

			for(j=0;j<2;j++)
				fscanf(fp, "%*f");
      
			fscanf(fp, "%f", &(color[i].r));
			color[i].r /= 255.0;
			fscanf(fp, "%f", &(color[i].g));
			color[i].g /= 255.0;
			fscanf(fp, "%f", &(color[i].b));
			color[i].b /= 255.0;
		}
		...
		*clist = malloc(sizeof(Color) * numPoly);
		...
		for(i=0;i<numPoly;i++) {
			...
			p[i].nVertex = nv;
//			p[i].zBufferFlag = 1;
			p[i].normals = malloc(sizeof(Vector)*nv);
			p[i].vertex = malloc(sizeof(Point)*nv);
			tcolor.r = tcolor.g = tcolor.b = 0.0;
			//      printf("%d: ", nv);
			for(j=0;j<nv;j++) {
				//	printf("%d  ", vid[j]);
				p[i].vertex[j] = vertex[vid[j]];
				if(!estNormals) {
					p[i].normals[j] = normal[vid[j]];
				}
				tcolor.r += color[vid[j]].r;
				tcolor.g += color[vid[j]].g;
				tcolor.b += color[vid[j]].b;
			}
			tcolor.r /= (float)nv;
			tcolor.g /= (float)nv;
			tcolor.b /= (float)nv;
			...
			if(estNormals) {
				...
				tx.x = p[i].vertex[0].x - p[i].vertex[1].x;
				tx.y = p[i].vertex[0].y - p[i].vertex[1].y;
				tx.z = p[i].vertex[0].z - p[i].vertex[1].z;


				ty.x = p[i].vertex[2].x - p[i].vertex[1].x;
				ty.y = p[i].vertex[2].y - p[i].vertex[1].y;
				ty.z = p[i].vertex[2].z - p[i].vertex[1].z;
